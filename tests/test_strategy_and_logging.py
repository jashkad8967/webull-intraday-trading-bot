...
class PaperWebullAPITests(unittest.TestCase):
    STATE_PATH = Path("tests/.generated_paper_state/ledger.json")
    ...
    def _config(self, **overrides):
        kwargs = dict(
            ...
            mode="PAPER",
            paper_starting_balance=Decimal("250"),
    ...
    def setUp(self):
        shutil.rmtree(self.STATE_PATH.parent, ignore_errors=True)
        # WebullAPI.__init__ builds a real TradeClient, which authenticates
        # against Webull over the network - stub both SDK clients so paper
        # mode's unit tests never depend on network access or real creds.
        self._trade_patch = unittest.mock.patch(
            "webull.trade.trade_client.TradeClient",
            return_value=SimpleNamespace(
                order_v3=SimpleNamespace(place_order=unittest.mock.Mock())
            ),
        )
        self._data_patch = unittest.mock.patch(
            "webull.data.data_client.DataClient", return_value=SimpleNamespace()
        )
        self._trade_patch.start()
        self._data_patch.start()

    def tearDown(self):
        self._trade_patch.stop()
        self._data_patch.stop()
        shutil.rmtree(self.STATE_PATH.parent, ignore_errors=True)

    def test_starts_seeded_at_the_configured_balance_with_no_positions(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config(paper_starting_balance=Decimal("500")))

        self.assertEqual(api.buying_power(), Decimal("500"))
        self.assertEqual(api.positions(), [])

    def test_buy_fill_updates_cash_and_opens_a_position(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config())
        order_id = api.place_stock(
            "AAPL", "BUY", Decimal("2"), limit_price=Decimal("100")
        )

        self.assertTrue(order_id)
        self.assertEqual(api.buying_power(), Decimal("50"))
        positions = {p["symbol"]: p for p in api.positions()}
        self.assertEqual(positions["AAPL"]["instrument_type"], "EQUITY")
        self.assertEqual(Decimal(positions["AAPL"]["quantity"]), Decimal("2"))
        self.assertEqual(Decimal(positions["AAPL"]["cost_price"]), Decimal("100"))

    def test_sell_fill_closes_the_position_and_credits_cash_minus_fee(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config())
        api.place_stock("AAPL", "BUY", Decimal("2"), limit_price=Decimal("100"))
        api.place_stock("AAPL", "SELL", Decimal("2"), limit_price=Decimal("110"))
        ...
        self.assertEqual(api.buying_power(), expected_cash)

    def test_buy_rejected_when_it_would_exceed_simulated_cash(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config(paper_starting_balance=Decimal("10")))

        with self.assertRaises(RuntimeError):
            api.place_stock("AAPL", "BUY", Decimal("1"), limit_price=Decimal("100"))
        self.assertEqual(api.buying_power(), Decimal("10"))
        self.assertEqual(api.positions(), [])

    def test_option_fill_uses_the_100x_contract_multiplier(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config(paper_starting_balance=Decimal("1000")))
        contract = {"symbol": "AAPL240119C00100000"}
        api.place_option(contract, "BUY", 1, Decimal("2"), "OPEN")

        self.assertEqual(api.buying_power(), Decimal("800"))
        positions = {p["symbol"]: p for p in api.positions()}
        self.assertEqual(positions[contract["symbol"]]["instrument_type"], "OPTION")

    def test_open_orders_and_order_detail_reflect_immediate_fills(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config())
        order_id = api.place_stock(
            "AAPL", "BUY", Decimal("1"), limit_price=Decimal("100")
        )

        self.assertEqual(api.open_orders(), [])
        detail = api.order_detail(order_id)
        self.assertEqual(detail["orders"][0]["status"], "FILLED")
        self.assertEqual(api.cancel_all_orders(), [])

    def test_state_survives_a_reload_from_disk(self):
        from webull_bot.paper_broker import PaperWebullAPI

        config = self._config()
        first = PaperWebullAPI(config)
        first.place_stock("AAPL", "BUY", Decimal("2"), limit_price=Decimal("100"))

        second = PaperWebullAPI(config)
        self.assertEqual(second.buying_power(), Decimal("50"))
        positions = {p["symbol"]: p for p in second.positions()}
        self.assertEqual(Decimal(positions["AAPL"]["quantity"]), Decimal("2"))

    def test_never_touches_the_real_trade_client(self):
        from webull_bot.paper_broker import PaperWebullAPI

        api = PaperWebullAPI(self._config())
        api.place_stock("AAPL", "BUY", Decimal("1"), limit_price=Decimal("100"))
        api.place_option(
            {"symbol": "AAPL240119C00100000"}, "BUY", 1, Decimal("1"), "OPEN"
        )
        api.trade.order_v3.place_order.assert_not_called()
